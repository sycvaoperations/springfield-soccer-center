...
(function () {
  ...
  function renderBlock(b) {
    if (b.type === "h") return `<h4 class="pm-h">${esc(b.text)}</h4>`;
    if (b.type === "lead") return `<p class="pm-lead">${esc(b.text)}</p>`;
    if (b.type === "list")
      return `<ul class="pm-list">${b.items.map((i) => `<li>${esc(i)}</li>`).join("")}</ul>`;
  ...
  function openProgram(id) {
    ...
    if (!p) return;
    const d = p.details || {};
    const facts = d.facts || fallbackFacts(p);
    const body = d.body || [{ type: "lead", text: p.desc }];
